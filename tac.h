...
void print_quads_opt(){
    ...
    while(i < quadlen){
        if(i==while_line){
            while(inv_p < inv){
            if(q_inv[inv_p].op!=NULL) printf("%10s\t\t%20s\t%20s\t%20s\t\n", q_inv[inv_p].op, q_inv[inv_p].arg1, q_inv[inv_p].arg2, q_inv[inv_p].res);
            inv_p++;
            }
        }
        if(q[i].op!=NULL) printf("%10s\t\t%20s\t%20s\t%20s\t\n", q[i].op, q[i].arg1, q[i].arg2, q[i].res);
        i++;
    }
}
