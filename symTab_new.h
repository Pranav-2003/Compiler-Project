...
void insert(char *type, char *name, int lineno, int len, char *val){
	...
	if (l == NULL){
		...
		memset(l->value,0,sizeof(l->value));
...
Table_List *lookup_func(char *name){
	unsigned int hashval = hash(name);
	Table_List *l = hash_table[hashval];
	while ((l != NULL) && (strcmp(name,l->symbolTable_name) != 0) || (strcmp("Func_Name",l->symbolTable_type)!=0)) l = l->next;
...
void is_iter(char *name, int lineno){
	Table_List *l = lookup(name);
	if(strcmp(l->symbolTable_type, "List identifier") != 0 && strcmp(l->symbolTable_type, "Identifier") != 0){
...
void print_symbol_table(){
  ...
  for (i=0; i < HASHSIZE; ++i){
	if (hash_table[i] != NULL){
		...
		while (l != NULL){
			if(strcmp(l->symbolTable_type,"ICGtemporary")!=0){			
				Reference_List *t = l->lines;
				printf("%-20s ",l->symbolTable_name);
				printf("\t%-20s",l->symbolTable_type);
				printf("\t%-10s",l->value); 
				printf("\t  %d  ",l->scope);
				printf("\t\t");
				while (t != NULL){
